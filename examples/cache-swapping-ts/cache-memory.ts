 ...
 *
 * Notice: No explicit type annotations needed! TypeScript infers everything.
 */
export const memoryCacheResource = defineResource({
  start: () => {
    const cache = new Map<string, any>();
    const ttls = new Map<string, number>(); // Track TTLs
    const stats = {
      hits: 0,
      misses: 0,
      sets: 0,
    ...
    } satisfies CacheInterface & { _cleanup: () => void };
  },
  halt: async (cache) => {
    (cache as any)._cleanup();
    await cache.clear();

...
const queueResource = defineResource({
  dependencies: ["config", "redis"] as const,
  start: async ({
    config,
    redis,
  }: {
...
const jobProducerResource = defineResource({
  dependencies: ["config", "queue"] as const,
 ...
 */
async function main() {
  console.log("🧶 Starting Braided Queue Worker System (TypeScript)...\n");
  ...
  console.log("\n   # Or use Redis CLI");
  console.log("   docker exec -it braided-queue-redis-ts redis-cli");
  console.log("   > KEYS *");
